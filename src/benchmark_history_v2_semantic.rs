...
const SEMANTIC_CENSUS_CONTRACT: &str = "sniffbench-historical-v2-compiler-semantic-census-v21";
...
#[path = "benchmark_history_v2_semantic_assembly.rs"]
mod assembly;
...
fn build_semantic_snapshot_from_sets(
    ...
    .map_err(assembly::SemanticSnapshotAssemblyError::into_detail)
}
...
struct SemanticVariantInputs<'a> {
    ...
    indexer_census: &'a HistoricalV2SemanticIndexerVariantCensus,
}
...
fn process_semantic_variant(
    ...
    indexers.push(indexer_census.clone());
    Ok(())
}
...
fn build_semantic_snapshot_from_index_sets(
    ...
    progress: Option<assembly::SemanticContributionProgress<'_>>,
) -> Result<HistoricalV2SemanticSnapshotCensus, assembly::SemanticSnapshotAssemblyError> {
    assembly::build_semantic_snapshot_from_sets(
        root,
        source,
        files,
