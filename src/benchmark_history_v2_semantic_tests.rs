...
fn typescript_default_identifier_binds_the_exact_compiler_reference() {
    ...
    let binding = snapshot
        ...
        .iter()
        .find(|binding| binding.declaration_unit_id == declaration.declaration_unit_id)
        .expect("default identifier compiler binding");
    ...
    assert_eq!(binding.compiler_anchor.start_line_zero_based, 1);
    assert_eq!(binding.compiler_anchor.start_character_zero_based, 15);
    assert_eq!(binding.compiler_anchor.end_line_zero_based, 1);
    assert_eq!(binding.compiler_anchor.end_character_zero_based, 18);
    validation::validate_snapshot(
        &fixture.source,
        &snapshot,
        &changed_indexers,
...
#[test]
fn go_package_root_excludes_nonrequired_generated_sources() {
    let mut fixture = compiler_surface_fixture(
        "go",
        SemanticIndexerKind::Go,
        SemanticPositionEncoding::Utf8,
    ...
    let generated = fixture
        ...
        .source_files
        .iter_mut()
        .find(|file| file.repository_path == "zz_generated.deepcopy.go")
        .unwrap();
    ...
    fixture
        ...
        .unwrap()
        .documents
        .remove(&RepositoryPath("zz_generated.deepcopy.go".to_string()));
    let changed_indexers = BTreeSet::from([SemanticIndexerKind::Go]);
    let required_paths = fixture_required_paths(&fixture.source);
    ...
    assert_eq!(semantic.go_package_roots.len(), 1);
    assert_eq!(
        semantic.go_package_roots[0].source_repository_paths,
        ["api.go"]
    );
    assert!(
        semantic.go_package_roots[0]
