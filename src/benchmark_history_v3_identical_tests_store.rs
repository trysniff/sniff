use super::super::{
    HistoricalV3Materialization, HistoricalV3MechanicalQualification, HistoricalV3RankArtifactKind,
    HistoricalV3RankJournalError, HistoricalV3RankJournalErrorKind, HistoricalV3RankStage,
    HistoricalV3RankStageOutcome, HistoricalV3SemanticCensus, HistoricalV3SourceCensus,
    HistoricalV3StoredRankStage, HistoricalV3TestRecipe,
};
use serde::de::DeserializeOwned;

pub(super) struct IdenticalTestInputs {
...
) -> Result<IdenticalTestInputs, HistoricalV3RankJournalError> {
    if history.len() < 5 {
        return Err(invalid(
            "historical-v3 identical tests require five completed prior stages",
        ));
    }
    Ok(IdenticalTestInputs {
...
fn read_completed<T: DeserializeOwned>(
    ...
    if !matches!(
        ...
        return Err(invalid(format!(
            "historical-v3 identical tests require completed {label}"
        )));
    }
    stored
        .read_artifact()
