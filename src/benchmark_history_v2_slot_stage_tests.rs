...
#[test]
fn semantic_census_artifact_is_disk_backed_and_typed_on_demand() {
    let temp = tempfile::tempdir().unwrap();
    let state = temp.path().join("state");
    let expected = json!({"stage": "SemanticCensus", "evidence": [1, 2, 3]});
    {
        let mut journal = HistoricalV2SlotStageJournal::open(&state, "rust", 1).unwrap();
        ...
        journal
            .append(
                checkpoint_input(
                    ...
                    completed(HistoricalV2StageArtifactKind::SemanticCensus),
                ),
                Some(&expected),
            )
            .unwrap();

        assert!(journal.history()[4].artifact.is_none());
        assert_eq!(
            journal.history()[4]
    ...
    assert!(resumed.history()[4].artifact.is_none());
    assert_eq!(
        resumed.history()[4]
            .read_artifact::<serde_json::Value>()
            .unwrap(),
